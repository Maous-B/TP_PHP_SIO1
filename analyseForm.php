<?php
    ...
    if(isset($_POST['soumettre_final'])){
        ...
        $r = "SELECT p.ID_PROF
        FROM professeurs p
        WHERE p.ADRESSE_MAIL = :a_m";

        $q = $db->prepare($r);

        $q->bindParam(':a_m', $_SESSION['add_mail']);

        $q->execute();

        $donnees = $q->fetch();
        
        foreach($donnees as $ind => $value){
            global $id_prof;
            $id_prof = $value;
        }
        ...
        foreach($id_notes as $id => $notes){
            ...
            epreuve_ccf(DATE_EPREUVE, HEURE_CONVOCATION, DUREE_EPREUVE, NOTE_OBTENU, COEFFICIENT, COMMENTAIRE_EVALUATION, fk_clef_eleve, fk_id_prof, fk_id_ccf, fk_id_bts) 
            VALUES(:date_e, :heure_conv, :duree_e, :note_o, :coef, :com_eval, :fk_c_e, :fk_i_p, :fk_i_c, :fk_i_b);";
            ...
            $q_final->bindParam(':fk_i_p', $id_prof, PDO::PARAM_INT);
